...
void merge_graph_3lines(){
  ...
  */
  ...
  //  if ( i== 2-1 ||i== 3-1 || i== 4-1 || i== 6-1 ||i== 8-1 || i ==9-1 ) continue;// 3 lenses comparison
    if ( i== 1-1 || i== 2-1 ||i== 3-1 || i== 4-1 || i== 6-1 || i==  7-1 ||i== 8-1 || i ==9-1 ) continue;
    ...
    if (i ==1-1) PhtVsEff_Mag->SetTitle("50 + 30mm lens");
    if (i ==5-1) PhtVsEff_Mag->SetTitle("30 + 30mm lens");
    if (i ==7-1) PhtVsEff_Mag->SetTitle("50 + 50mm lens");
    ...
    if (i ==1-1) MagVsPht->SetTitle("50 + 30mm lens");
    if (i ==5-1) MagVsPht->SetTitle("30 + 30mm lens");
    if (i ==7-1) MagVsPht->SetTitle("50 + 50mm lens");
    mg_MagVsPht->Add(MagVsPht);

    if (i ==1-1) PhtVsPos->SetTitle("50 + 30mm lens");
    if (i ==5-1) PhtVsPos->SetTitle("30 + 30mm lens");
    if (i ==7-1) PhtVsPos->SetTitle("50 + 50mm lens");
    mg_PhtVsPos->Add(PhtVsPos);

    mg_PhtVsEff_Mag->Add(PhtVsEff_Mag);
    mg_LensAccMagVsPos->Add(LensAccMagVsPos);

    if (i ==1-1) MagVsPos->SetTitle("50 + 30mm lens");
    if (i ==5-1) MagVsPos->SetTitle("30 + 30mm lens");
    if (i ==7-1) MagVsPos->SetTitle("50 + 50mm lens");
...
PhtVsEff_Mag
  ...
  */
  ...
  TCanvas *c_PhtVsPos = new TCanvas("c_PhtVsPos","Phot/Ch vs Pos", 600,600);
  c_PhtVsPos->cd();
  mg_PhtVsPos->GetXaxis()->SetTitle("Position [mm]");
  mg_PhtVsPos->GetYaxis()->SetTitle("Photons per channel");
  mg_PhtVsPos->Draw("AL*");//[/code]
  c_PhtVsPos->BuildLegend();

  TCanvas * c_PhtVsEff_Mag = new TCanvas("c2","", 600,600);
  mg_PhtVsEff_Mag->Draw("AL*");
  c_PhtVsEff_Mag->BuildLegend();

  TCanvas * c_LensAccMagVsPos = new TCanvas("c3","", 600,600);
  mg_LensAccMagVsPos->Draw("AL*");
  c_LensAccMagVsPos->BuildLegend();

  TCanvas * c_MagVsPos = new TCanvas("c4"," Mag Vs Pos", 600,600);
  mg_MagVsPos->GetXaxis()->SetTitle("Position [mm]");
  mg_MagVsPos->GetYaxis()->SetTitle("Magnitude");
  mg_MagVsPos->Draw("AL*");
  c_MagVsPos->BuildLegend();

  TCanvas * c_LensAccVsPos = new TCanvas("c5","", 600,600);
  mg_LensAccVsPos->Draw("AL*");
  c_LensAccVsPos->BuildLegend();

  TCanvas * c_AberrVsPos = new TCanvas("c6","", 600,600);
  mg_AberrVsPos->GetXaxis()->SetTitle("Position [mm]");
  mg_AberrVsPos->GetYaxis()->SetTitle(" Mean Deviation From True Point [mm] ");
  mg_AberrVsPos->Draw("AL*");
  c_AberrVsPos->BuildLegend();

  TCanvas * c_MagVsPht = new TCanvas("c7","", 600,600);
